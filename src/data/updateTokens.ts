    ...
    try {
      ...
      for (const item of parsed) {
        if (item.exch_seg === 'NSE') {
          // Angel symbols usually end with -EQ for equities
          const normalizedSymbol = item.symbol.endsWith('-EQ') ? item.symbol : `${item.symbol}-EQ`;
          tokenMap[normalizedSymbol] = item.token;
